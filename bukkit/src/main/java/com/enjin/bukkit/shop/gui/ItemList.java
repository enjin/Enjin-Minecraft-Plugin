...
public class ItemList extends Menu {
    ...
    private void init(Menu parent, Shop shop, List<Item> items) {
        ...
        for (Item item : items) {
            ...
            MenuItem menuItem = new MenuItem(name.substring(0, name.length() >= 32 ? 32 : name.length()), new MaterialData(material == null ? Material.PAPER : material, item.getIconDamage().byteValue())) {
                @Override
                public void onClick(Player player) {
