    ...
    public static void submit(Player player, int moduleId, List<QuestionResponse> responses) {
        ...
        for (QuestionResponse response : responses) {
            if (response.getQuestion().getSystem() == 1) {
                if (subject == null && (response.getQuestion()
                                                .getType() == QuestionType.select || response.getQuestion()
                                                                                             .getType() == QuestionType.text)) {
                    subject = response;
        ...
        for (QuestionResponse response : answers) {
            if (response.getQuestion().getSystem() == 0) {
                if (response.getAnswer() instanceof String) {
                    ...
                    extra.add(new ExtraQuestion(response.getQuestion().getType(),
                                                response.getQuestion().getId(),
                                                response.getQuestion().getLabel(),
                                                answer));
        ...
        RPCData<RPCSuccess> result = service.createTicket(moduleId,
                                                          (String) subject.getAnswer(),
                                                          (String) description.getAnswer(),
                                                          player.getName(),
                                                          extra);
        if (player != null) {
            if (result != null) {
