    ...
    public void processCommand(CommandSender sender, String command, Event event) {
        ...
        if (command.toLowerCase().startsWith("pex group ")) {
            ...
            if (args.length > 5 && PermissionsUtil.hasPermission(sender, "permissions.manage.membership." + args[2])) {
                if (args[3].equalsIgnoreCase("user") && (args[4].equalsIgnoreCase("add") || args[4].equalsIgnoreCase(
                        "remove") || args[4].equalsIgnoreCase("set"))) {
            ...
            if (args.length > 5 && PermissionsUtil.hasPermission(sender, "permissions.manage.membership." + args[5])) {
                if (args[3].equalsIgnoreCase("group") && (args[4].equalsIgnoreCase("add") || args[4].equalsIgnoreCase(
                        "remove") || args[4].equalsIgnoreCase("set"))) {
                    String        ep = args[2];
                    OfflinePlayer op = Bukkit.getOfflinePlayer(ep);
                    //We need to make sure the value executes before we actually grab the data.
