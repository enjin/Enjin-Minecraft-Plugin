...
public class StatsPlayer {
    ...
    public void addBrokenBlock(net.canarymod.api.world.blocks.Block block) {
        brokenblocks++;
    ...
    public void addPlacedBlock(net.canarymod.api.world.blocks.Block block) {
        placedblocks++;
        String blockid = block.getTypeId() + "-" + block.getData();
        int blocksplaced = 0;
    ...
    public synchronized EnjinStats.Server.Player getSerialized() {
        ...
        for (Entry<EntityType, Integer> ent : creaturekills.entrySet()) {
            stats.addPveentitykills(EnjinStats.Server.Player.PveKills.newBuilder().setMob(ent.getKey().name()).setKills(ent.getValue()).build());
        }
