    ...
    public void run() {
        ...
        try {
            ...
            if (destination.delete() && tempfile.renameTo(destination)) {
                ...
                EnjinMinecraftPlugin.logger.warning("[Enjin Minecraft Plugin] Enjin Minecraft plugin was updated to version " + versionnumber + ". Please restart your server.");
                return;
            } else {
                plugin.updatefailed = true;
                EnjinMinecraftPlugin.logger.warning("[Enjin Minecraft Plugin] Unable to update to new version. Please update manually!");
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
