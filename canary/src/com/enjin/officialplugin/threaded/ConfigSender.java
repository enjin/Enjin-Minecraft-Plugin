		...
		//Only run the ssl test on first run.
				if(EnjinMinecraftPlugin.usingSSL) {
					if(!plugin.testHTTPSconnection()) {
						...
						EnjinMinecraftPlugin.logger.warning("SSL test connection failed, The plugin will use http without SSL. This may be less secure.");
				...
				try {
					...
					if(success.equalsIgnoreCase("ok")) {
						if(plugin.unabletocontactenjin) {
							...
							for(Player player : players) {
								if(player.hasPermission("enjin.notify.connectionstatus")) {
									...
									EnjinMinecraftPlugin.logger.info("Connection to Enjin re-established!");
								}
							}
						}
						...
						EnjinMinecraftPlugin.logger.warning("Auth key invalid. Please regenerate on the enjin control panel.");
						plugin.stopTask();
						ArrayList<Player> players = Canary.getServer().getPlayerList();
						for(Player player : players) {
						...
						EnjinMinecraftPlugin.logger.warning("Oops, we sent bad data, please send the enjin.log file to enjin to debug.");
						...
						EnjinMinecraftPlugin.logger.info("Enjin said to wait, will retry at next boot.");
						...
						EnjinMinecraftPlugin.logger.info("Enjin is having something going on, if you continue to see this error please report it to enjin.");
					}else {
						...
						EnjinMinecraftPlugin.logger.info("Something happened on sync, if you continue to see this error please report it to enjin.");
						EnjinMinecraftPlugin.logger.info("Response code: " + success);
					}
