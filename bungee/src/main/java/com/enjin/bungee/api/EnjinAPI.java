...
import com.enjin.bungee.points.ErrorConnectingToEnjinException;
import com.enjin.bungee.points.PlayerDoesNotExistException;
import com.enjin.bungee.points.PointsAPI;
...
public class EnjinAPI {
    ...
    public static ConcurrentHashMap<String, PlayerTag> getPlayerTags(String player) throws PlayerDoesNotExistException, ErrorConnectingToEnjinException {
        ...
        try {
            EnjinPlugin.debug("Connecting to Enjin to retrieve tags for player " + player);
            ...
            String json = PointsAPI.parseInput(in);

            EnjinPlugin.debug("Content of player tags query:\n" + json);
    ...
    public static ConcurrentHashMap<String, PlayerTag> getPlayerTags(UUID uuid) throws PlayerDoesNotExistException, ErrorConnectingToEnjinException {
        ConcurrentHashMap<String, PlayerTag> tags = new ConcurrentHashMap<String, PlayerTag>();
        try {
            EnjinPlugin.debug("Connecting to Enjin to retrieve tags for UUID " + uuid.toString());
            URL enjinurl = getPlayerTagsUrl();
            HttpURLConnection con;
            // Mineshafter creates a socks proxy, so we can safely bypass it
            // It does not reroute POST requests so we need to go around it
    ...
    private static URL getPlayerTagsUrl() throws Throwable {
        return new URL((EnjinPlugin.usingSSL ? "https" : "http") + EnjinPlugin.apiurl + "minecraft-get-tags");
    }
