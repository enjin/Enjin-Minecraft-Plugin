...
public class EnjinPointsSyncClass implements Runnable {
	...
	public synchronized void run() {
		try {
			...
			sender.sendChatToPlayer(ChatMessageComponent.func_111066_d(ChatColor.DARK_GREEN + "Successfully " + addremove + " " + points + " points to player " + playername + "! The player now has " + amount + " points."));
			...
			sender.sendChatToPlayer(ChatMessageComponent.func_111066_d(ChatColor.DARK_RED + "Enjin Error: Not a valid number!"));
		} catch (PlayerDoesNotExistException e) {
			sender.sendChatToPlayer(ChatMessageComponent.func_111066_d(ChatColor.DARK_RED + "Enjin Error: That player has not registered on the website yet! In order to use this feature the player must be added on the website."));
		} catch (ErrorConnectingToEnjinException e) {
			e.printStackTrace();
			sender.sendChatToPlayer(ChatMessageComponent.func_111066_d(ChatColor.DARK_RED + "Enjin Error: We're unable to connect to enjin at this current time, please try again later."));
		}
	}
